...
async function isMongoDBAvailable(): Promise<boolean> {
  try {
    ...
    console.warn(
      "MongoDB not available, using in-memory storage for development",
    );
    return false;
  }
}
...
export const getATSScore: RequestHandler = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const resume = await Resume.findById(id).select(
      "atsScore personalInfo.fullName",
    );
...
export const searchResumes: RequestHandler = async (req, res) => {
  try {
    ...
    if (minScore)
      query["atsScore.totalScore"] = { $gte: parseInt(minScore as string) };
    ...
    const resumes = await Resume.find(query)
      .select(
        "personalInfo.fullName personalInfo.email template atsScore.totalScore updatedAt",
      )
      .sort({ updatedAt: -1 })
      .limit(10);
